...
from . import _linear_features as _parse_features
from ._linear_features cimport CONTEXT_SIZE
from ._linear_features cimport fill_context
...
cdef class LinearParser:
    ...
    def Model(cls, nr_class, **cfg):
        cfg.setdefault('l1_penalty', 0.0)
        return ParserModel(cfg['features']), cfg
    ...
    def __init__(self, Vocab vocab, moves=True, model=True, **cfg):
        ...
        cfg['features'] = get_templates('parser')
        self.cfg = cfg
        self.model = model
        self.tagger = None
        if model not in (True, None):
            self._model = model
        self._multitasks = []

    def __reduce__(self):
        return (LinearParser, (self.vocab, self.moves, self.model), None, None)

    def __call__(self, Doc tokens, beam_width=None, beam_density=None):
    ...
    cdef int parseC(self, TokenC* tokens, int length, int nr_feat) nogil:
        ...
        while not state.is_final():
            eg.nr_feat = self._model.set_featuresC(eg.atoms, eg.features, state)
            self.moves.set_valid(eg.is_valid, state)
            self._model.set_scoresC(eg.scores, eg.features, eg.nr_feat)
    ...
    def update(self, docs, golds, drop=0., sgd=None, losses=None):
        if losses is None:
            ...
            losses.setdefault('parser', 0.)
        if not docs[0].is_tagged:
            docs = list(self.tagger.pipe(docs))
        for doc, gold in zip(docs, golds):
            losses['parser'] += self._update(doc, gold, drop=drop)

    def _update(self, Doc tokens, GoldParse gold, itn=0, double drop=0.0):
        """
        ...
        while not stcls.is_final():
            eg.c.nr_feat = self._model.set_featuresC(eg.c.atoms, eg.c.features,
                                                    stcls.c)
            dropout(eg.c.features, eg.c.nr_feat, dropout_rate)
            self.moves.set_costs(eg.c.is_valid, eg.c.costs, stcls, gold)
            self._model.set_scoresC(eg.c.scores, eg.c.features, eg.c.nr_feat)
            guess = VecVec.arg_max_if_true(eg.c.scores, eg.c.is_valid, eg.c.nr_class)
            self._model.update(eg)
    ...
    def begin_training(self, get_gold_tuples, pipeline=None, sgd=None, **cfg):
        ...
        for action, freqs in self.moves.labels.items():
            actions.setdefault(action, {}).update(freqs)
        ...
        self.tagger = [model for name, model in pipeline if name == 'tagger'][0]
        return sgd
    ...
    def use_params(self, params):
        with self.model.with_averages():
            yield

    def to_disk(self, path, **exclude):
        serializers = {
            'model': lambda p: self.model.dump(str(p)),
...
cdef class StepwiseState:
    ...
    cdef readonly LinearParser parser

    def __init__(self, LinearParser parser, Doc doc, GoldParse gold=None):
        self.parser = parser
        self.doc = doc
        if gold is not None:
            self.gold = gold
    ...
    def predict(self):
        self.eg.reset()
        self.eg.c.nr_feat = self.parser._model.set_featuresC(self.eg.c.atoms,
                                self.eg.c.features, self.stcls.c)
